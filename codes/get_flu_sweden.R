...
###################################
flu.df = flu %>%
  dplyr::filter(Week == week & County != 'WeeklyTotal') %>%
  mutate(Counts = Current.week.cases.A + Current.week.cases.B) %>%
  dplyr::select(Region = County, Laboratory, Counts, Samples) %>%
  left_join(y = cities, by = c('Laboratory'='city')) %>%
  dplyr::select(Region, Laboratory, Counts, Samples, Latitude=lat, Longitude=lng) 

flu.df[flu.df$Region == 'Stockholm',5] = 59.3294
flu.df[flu.df$Region == 'Stockholm',6] = 18.0686

## Keeping labs in Västra Götaland separated
# stockholm = flu.df %>%
#   dplyr::filter(Region == 'Stockholm') %>%
#   group_by(Region, Latitude, Longitude) %>%
#   summarise(Counts = sum(Counts), Samples = sum(Samples)) %>%
#   ungroup() %>%
#   mutate(Laboratory = 'Stockholm', .after=Region) %>%
#   as.data.frame()
# 
# flu.df.grouped = dplyr::filter(flu.df, Region != 'Stockholm') %>%
#   rbind(stockholm) %>%
#   dplyr::arrange(Region)
# flu.df.grouped.vg.labs = flu.df.grouped

#Otherwise, just focusing on regions
flu.df.grouped = flu.df %>%
  group_by(Region) %>%
  summarise(Latitude = mean(Latitude), Longitude = mean(Longitude), 
            Counts = sum(Counts), Samples = sum(Samples)) %>%
  ungroup() %>%
  as.data.frame()

flu.df.grouped = dplyr::left_join(x = flu.df.grouped, y = swe.shp@data, by = c('Region'='NAME_1')) %>%
  dplyr::select(Region, ID.Region = ID_1, Longitude, Latitude, Counts, Samples)

### REM: If you want labs, UNCOMMENT THE NEXT 3 LINES AND COMMENTS THE ABOVE 8 LINES
# flu.df.grouped = dplyr::left_join(x=flu.df.grouped, y=swe.shp@data, by=c('Region'='NAME_1')) %>%
#   dplyr::select(Region, Laboratory, ID.Region=ID_1, Longitude, Latitude, Counts, Samples)
# flu.df.grouped = mutate(flu.df.grouped, ID.District = 1:nrow(flu.df.grouped), .after = ID.Region)

flu.df.grouped$ID.Region = as.numeric(flu.df.grouped$ID.Region)
#ID.Region in flu.df.grouped is the same of the shp.file
#21 regions and 24 laboratories, 4 of which are in Västra Götaland

## Adding regional population 
...
cat('\nGetting regional flu counts of week', week, ': done!\n')
